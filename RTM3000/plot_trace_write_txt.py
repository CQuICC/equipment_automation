...
Short Desc: Python example to transfer waveform data from Rohde & Schwarz Oscilloscope.

#######################################################
Description: The code connected with R&S DSO over ethernet and collects the waveform and writes it in a txt file.
The example uses channel 2, however, it can easily be modified for all the channels. 


Enter the timescale in line#38;
It is possible to connect the instrument over USB. Uncomment #25 
"""
...
try:
    ...
    rth = RsInstrument('TCPIP::172.16.0.92::INSTR', True, False)		#localhost ip address
    #rth = RsInstrument('USB::0x0AAD::0x0119::022019943::INSTR' , True, False)  # USB-TMC (Test and Measurement Class))
